...
| 2026-08-26 | Align the report states with Doc 04 rather than the shipped schema | The enum carried an undocumented `reviewing` and lacked `restricted` and `escalated`. Doc 04 is a safety document and outranks the schema, so the code follows the document; an existing `reviewing` row means the same as `restricted` |
...
| 2026-08-26 | Recorded the report-state alignment with Doc 04 |
