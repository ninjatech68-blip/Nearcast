...
| 2026-08-26 | Delivered the moderation half of the gate (B-9): one queue over open reports and classifier-held content, `moderate_report` with the four documented decisions, `restore_intent` to lift a restriction back to its captured safe state, and an immutable audit row per action. Report states aligned to Doc 04's five. Clients lost `update`/`delete` on `reports` and all write access to `moderation_actions` after a test found a silent zero-row no-op where a refusal belonged. The pre-alpha safety gate is now complete at the database layer; moderator-facing UI and the founder's role assignment (H-8) remain |
