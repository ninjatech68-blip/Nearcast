...
| 2026-08-26 | Pre-alpha safety gate | Passed | New suite `nearcast_safety_gate.test.sql` at 21 of 21, taking the harness total to 212 across eight suites. `private.prohibited_category` classifies the four Doc 04 categories with word-bounded patterns, so "method" is not "meth"; a match restricts the intent pending review, keeps the safe state in `restricted_from`, and appends a `restricted_pending_review` event with a null actor — the system restricting content, not a fabricated reporter. Editing into prohibited content restricts too, so an edit is not a way past the check. `redeem_invite` now requires an adult affirmation and records it on `profile_private`; the two-argument form is dropped so no unaffirmed path remains callable. Rate limits added to publishing (12/hour) and messaging (60/hour), joining the existing limits on confirmation, response and report. Six call sites in two existing suites were updated to the new redemption contract |
| 2026-08-26 | Honest publish outcome | Passed | A restricted intent is no longer reported as published: the review screen says it is being reviewed, keeps the draft, generates no deliveries, and does not navigate away. 1 new screen assertion; `npm run verify` clean with 14 Vitest files (103 tests), 15 Jest suites (77 tests), iOS export |
| 2026-08-26 | Moderation queue | Passed | New suite `nearcast_moderation.test.sql` at 25 of 25, taking the harness total to 237 across nine suites. One queue covers both reports and content the classifier held, so nothing is restricted and then forgotten. Every decision writes an immutable audit row with actor, action, reason and the subject's captured state, and a restriction can be lifted back to the state it was restricted from. A moderator claim placed in user metadata grants nothing — authorisation reads `app_metadata` only, and there is a test for exactly that. The report states now match Doc 04's five |
| 2026-08-26 | Report evidence hardened | Fixed | The suite found that a reporter attempting to change their own report's status silently affected zero rows instead of being refused: `update` was granted with no matching policy. Clients no longer hold `update` or `delete` on `reports`, and hold nothing at all on `moderation_actions`. A silent no-op is a worse answer than a refusal, and a report is evidence |
...
| 2026-08-26 | Delivered the pre-alpha safety gate: prohibited-content restriction, the minors gate, and rate limits on creation and messaging |
