...
const none: AlertCounts = { needs: 0, news: 0, plans: 0 };
...
describe('alertTabs', () => {
  ...
  it('never reorders: a tab appears at its own position, others do not move', () => {
    // news arriving must slot BETWEEN needs and plans, not on the end.
    expect(alertTabs({ needs: 1, news: 0, plans: 1 }, 'needs').visible).toEqual(['needs', 'plans']);
    expect(alertTabs({ needs: 1, news: 4, plans: 1 }, 'needs').visible).toEqual(['needs', 'news', 'plans']);
    expect(alertTabs({ needs: 0, news: 4, plans: 1 }, 'news').visible).toEqual(['news', 'plans']);
  });

  it('hides the strip below two populated groups', () => {
    expect(alertTabs({ ...none, news: 5 }, 'news').showStrip).toBe(false);
    expect(alertTabs({ ...none, news: 5, plans: 1 }, 'news').showStrip).toBe(true);
  });
  ...
  it('falls forward when the selected tab empties under you', () => {
    ...
    expect(alertTabs({ needs: 0, news: 2, plans: 1 }, 'needs').shown).toBe('news');
    // and forward means the FIRST tab still holding rows, never the last
    expect(alertTabs({ needs: 0, news: 0, plans: 1 }, 'needs').shown).toBe('plans');
  });

  it('keeps the selected tab while it still holds rows', () => {
    expect(alertTabs({ needs: 2, news: 2, plans: 2 }, 'plans').shown).toBe('plans');
  });

  it('declares one fixed order for the whole app', () => {
    expect(ALERT_TAB_ORDER).toEqual(['needs', 'news', 'plans']);
  });
});
